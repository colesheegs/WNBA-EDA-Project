...
home_game_results <- close_games_observing |> 
  ungroup() |> 
  group_by(home_team_name, home_team_outcome) |>
  summarise(num_times_occured = n()) |> 
  rename(team_name = home_team_name) |> 
  mutate(win_or_loss = if_else(str_detect(home_team_outcome, "^Won"), "Win",
...
num_home_outcomes <- close_games_observing |>
  ungroup() |> 
  select(home_team_outcome) |> 
  count(home_team_outcome) |> 
...
away_game_results <- close_games_observing |> 
  ungroup() |> 
  group_by(away_team_name, away_team_outcome) |> 
  summarise(times_occured = n()) |> 
  rename(team_name = away_team_name) |> 
  mutate(win_or_loss = if_else(str_detect(away_team_outcome, "^Won"), "Win",
...
num_away_outcomes <- close_games_observing |>
  ungroup() |> 
  select(away_team_outcome) |> 
  count(away_team_outcome) |> 
...
possible_outcomes <- full_join(num_home_outcomes, num_away_outcomes, join_by(home_team_outcome == away_team_outcome))



possible_outcomes[is.na(possible_outcomes)] <- 0
...
possible_outcomes <- possible_outcomes |> 
  mutate(outcome = factor(possible_outcomes$outcome, levels = c("Lost - Lost Lead", "Lost - Deficit Increased", "Lost - No Comeback", "Lost - Deficit Decreased", "Lost - From Tie", "Tied - Lost Lead", "Tied - No Change", "Tied - Comeback", "Won - Lead Decreased", "Won - Kept Same Lead", "Won - Extended Lead", "Won - From Tie", "Won - Took Lead From Loss")))
...
win_losses <- close_games_observing |>
  mutate(Home = if_else(str_detect(home_team_outcome, "^Won"), "Win",
                       if_else(str_detect(home_team_outcome, "^Lost"), "Loss", "Tie")),
         Away = if_else(str_detect(away_team_outcome, "^Won"), "Win",
                       if_else(str_detect(away_team_outcome, "^Lost"), "Loss", "Tie"))) |> 
  pivot_longer(Home:Away,
...
home_outcomes <- ggplot(possible_outcomes, aes(x = win_or_loss, y = num_times_home, fill = outcome)) +
...
away_outcomes <- ggplot(possible_outcomes, aes(x = win_or_loss, y = num_times_away, fill = outcome)) +
  geom_bar(stat = "identity", position = "dodge") +
