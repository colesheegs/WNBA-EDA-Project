...
# Removed The All Star Game & Added Shooter Variable & Added Variables
```{r, include=FALSE}
...
all_close_game_shots <- left_join(all_close_games, all_shots) |> 
  mutate(made_shot = factor(all_close_game_shots$made_shot,levels = c("Missed", "Made"))) 
...
league_win_vs_losses <- ggplot(win_losses, aes(x = home_or_away_result, fill = win_or_loss)) +
  ...
  labs(title = "Outcomes For Home And Away Teams In Close Games",
       x = "Home vs. Away",
       fill = "Result") 
   # geom_label(labels = fill, color = "white",
   #          stat = "count",
...
## Number of Close Home And Away Games (6 points or less with 30 seconds left) Per Team
```{r}
...
ggplot(win_losses2, aes(x = fct_rev(fct_infreq(teams)), fill = home_or_away_team)) +
...
## Each Team's Shooting Percentage (Home and Away)
```{r}
ggplot(all_close_game_shots, aes(x = Shooting_Team_Home_Away)) +
  geom_bar(aes(fill = made_shot), color = "black", position = "fill") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
...
## Each Team's Outcome When Home vs. Away
## Outcomes Related To Home and Away
```{r, include=FALSE}
...
# Number of Occurences of All Outcomes For Home vs. Away
possible_outcomes2 <- full_join(home_game_results, away_game_results, by = "team_name") 
...
win_losses |> 
  filter(teams == "Dallas" & home_or_away_team == home_or_away_outcome & home_or_away_team == home_or_away_result & home_or_away_result == home_or_away_outcome)
  

 
  ggplot(aes(x = Win_Loss, y = num_times_away, fill = outcome)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Occurences of Each Outcome Type For Away Teams",
       y = "Occurences",
       fill = "Outcome") +
...
# |> 
#   ggplot(aes(x = win_or_loss)) +
#   facet_wrap(~home_or_away_outcome)
...
shots_made_data1 |> 
  ggplot(aes(x = fct_rev(fct_infreq(home)), fill = made_shot))  +
  geom_bar(color = "black") +
  theme(axis.text.x=element_text(angle=45, hjust=0.9)) +
  scale_fill_manual(values = c("#E77500", "white")) +
  coord_flip() +
