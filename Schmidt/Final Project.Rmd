...
all_close_game_shots |> 
    filter(shooting_team == input$teams6) |>
ggplot(aes(x = made_shot, fill = made_shot))  +
  geom_bar(color = "black") +
  scale_fill_manual(values = c("white", "#E77500")) +
    theme(panel.background = element_rect(fill = "gray80"), panel.grid.minor = element_blank()) +
...
win_losses2 <- close_games_observing |>
 ...
 inputPanel(
    checkboxGroupInput("teams_check", "Select Teams For Wins vs. Losses:",
...
renderPlot({
    filtered_teams1 <- if("All" %in% input$teams_check) {
      win_loss_per_team
    } else {
      win_loss_per_team |>
        filter(team %in% input$teams_check)
    }
