...
#win_loss_per_team
  ...
  inputPanel(
    checkboxGroupInput("teams_check1", "Select Teams For Graph 1:",
...
win_losses2 <- close_games_observing |>
 ...
 inputPanel(
    checkboxGroupInput("teams_check", "Select Teams For Graph 2:",
...
```{r, echo=FALSE}
 ...
 inputPanel(
    checkboxGroupInput("teams_check2", "Select Teams:",
...
```{r}
renderPlot({
    filtered_teams4 <- if("All" %in% input$teams_check2) {
      all_close_game_shots
    } else {
      all_close_game_shots |>
        filter(shooting_team %in% input$teams_check2)
    }

ggplot(filtered_teams4, aes(x = Shooting_Team_Home_Away)) +
  geom_bar(aes(fill = made_shot), color = "black", position = "fill") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  scale_fill_manual(values = c("white", "#E77500")) +
